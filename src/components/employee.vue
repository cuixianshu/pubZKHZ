...
<div class="father">
  ...
  <div id="clientDepartment" class="container-fluid">
    <div class="row form-inline query">
      <button @click="getListOfEmployees" class="btn btn-primary">获取员工名册</button>
      <button @click="clearlistOfEmployees"class="btn btn-secondary" v-if="listOfEmployees.length>0">清空</button>
      <button @click="newCreateEmployee" class="btn btn-primary newitem">新建员工</button>
    </div>
    <div class="divfortable" v-if="listOfEmployees.length>0">
          ...
          <div class="container-fluid">
            <div class="row">
              <div class="col-lg form-inline">
                ...
                <input id="inputNameOfEmployee" type="text" class="form-control" v-model="employee.name" placeholder="员工姓名" title="员工姓名">
              </div>
              <div class="col-lg form-inline">
                <label for="inputGender">员工性别:</label>
                <select id="inputGender" type="text" class="form-control" v-model="employee.gender" placeholder="性别" title="性别">
            ...
            <div class="row"> 
              <div class="col-lg form-inline">
                ...
                <input id="inputBornDate" type="date" class="form-control" v-model="employee.born_date" placeholder="出生日期" title="出生日期">
              </div>
              <div class="col-lg form-inline">
                <label for="inputIdcard">身份证号:</label>
                <input id="inputIdcard" type="text" class="form-control" v-model="employee.idcard" placeholder="身份证号" title="身份证号">
              </div>
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputEducation">最高学历:</label>
                <input id="inputEducation" type="text" class="form-control" v-model="employee.education" placeholder="最高学历" title="最高学历">
              </div> 
              <div class="col-lg form-inline">
                <label for="inputGraduateFrom">毕业院校:</label>
                <input id="inputGraduateFrom" type="text" class="form-control" v-model="employee.graduate_from" placeholder="毕业院校" title="毕业院校">
              </div>                                        
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputAddress">家庭住址:</label>
                <input id="inputAddress" type="text" class="form-control" v-model="employee.address" placeholder="家庭住址" title="家庭住址">
              </div>
              <div class="col-lg form-inline">
                <label for="inputPrvtTel">个人电话:</label>
                <input id="inputPrvtTel" type="text" class="form-control" v-model="employee.tel_private" placeholder="个人电话" title="个人电话">
              </div>              
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputWorkTel">办公电话:</label>
                <input id="inputWorkTel" type="text" class="form-control" v-model="employee.tel_work" placeholder="办公电话" title="办公电话">
              </div>
              <div class="col-lg form-inline">
                <label for="inputEmrgcCntactr">紧急联系:</label>
                <input id="inputEmrgcCntactr" type="text" class="form-control" v-model="employee.emergency_contacter" placeholder="紧急联系人" title="紧急联系人">
              </div>              
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputTelInEmrgc">紧急电话:</label>
                <input id="inputTelInEmrgc" type="text" class="form-control" v-model="employee.tel_emergency" placeholder="紧急联系电话" title="紧急联系电话">
              </div>
              <div class="col-lg form-inline">
                <label for="inputJoinDate">入职日期:</label>
                <input id="inputJoinDate" type="date" class="form-control" v-model="employee.date_join" placeholder="入职日期" title="入职日期" :readonly="!isNewCreated">
              </div>              
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputAddress">所在部门:</label>
                <select id="inputAddress" type="text" class="form-control" v-model="employee.id_department" placeholder="所在部门" title="所在部门">
                  <option v-for="item in departments" :value="item.id">{{item.name}}</option>
                </select>
              </div>
              <div class="col-lg form-inline">
                <label for="inputPosition">担任职务:</label>
                <input id="inputPosition" type="text" class="form-control" v-model="employee.position" placeholder="担任职务" title="担任职务">
              </div>              
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputCrtfctRnk">等级证书:</label>
                <input id="inputCrtfctRnk" type="text" class="form-control" v-model="employee.certificate_and_rank" placeholder="等级证书" title="等级证书">
              </div>
              <div class="col-lg form-inline">
                <label for="inputRemark">备注信息:</label>
                <input id="inputRemark" type="text" class="form-control" v-model="employee.remark" placeholder="备注信息" title="备注信息">
              </div>
            </div>
            <div class="row" v-show="!isNewCreated">
              <div class="col-lg form-inline">
                <label for="inputLeaveDate">离职日期:</label>
                <input id="inputLeaveDate" type="date" class="form-control" v-model="employee.date_leave" placeholder="离职日期" title="离职日期">
              </div>              
              <div class="col-lg form-inline" v-show="!isNewCreated">
                <label for="inputWhyLeave">离职原因:</label>
                <input id="inputWhyLeave" type="text" class="form-control" v-model="employee.why_leave" placeholder="离职原因" title="离职原因">
              </div>
            </div>
            <div class="row" v-show="!isNewCreated">
              <div class="col-lg form-inline">
                <label for="slctCreater">建档员工:</label>
                <select id="slctCreater" type="text" class="form-control" v-model="employee.id_creater" placeholder="建档员工" title="建档员工" disabled>
                  <option v-for="item in operators" :value="item.id">{{item.name}}</option>
                </select>
              </div>
              <div class="col-lg form-inline">
                <label for="inputTimeCreate">建档日期:</label>
                <input id="inputTimeCreate" type="date" class="form-control" v-model="employee.time_create" placeholder="建档日期" title="建档日期" readonly>
              </div>
            </div>
            <div class="row">
              <div class="col-lg form-inline">
                <label for="inputFrom">来自哪里:</label>
                <input id="inputFrom" type="text" class="form-control" v-model="employee.from_ognztn" placeholder="来自哪里" title="来自哪里">
              </div>
              <div class="col-lg form-inline">
                <label for="inputIsOwn">是否自有:</label>
                <input id="inputIsOwn" type="text" class="form-control" v-model="employee.is_own" placeholder="是否自有" title="1:自有;0:外协">
              </div>
            </div>            
          </div>
        </div>
