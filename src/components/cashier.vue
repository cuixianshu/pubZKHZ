    ...
    <div id="queryOfCashier" class="container-fluid">
      <div class="row">
        <div class="col-lg form-inline searchcontent">
          ...
          <button id="byhand" @click="" class="btn btn-primary" type="button">手工填表</button>
        </div>          
      </div>
              ...
              <div class="row">
                <div class="col-lg  form-inline">
                  ...
                  <select id="slctCashierAccount" type="text" name="cashierAccount" class="form-control" placeholder="收款账号" v-model="cashier.account" title="收款账号">
                  	<option v-for="item in ourAccount">{{item.short_name}}</option>}
                  </select>
                </div>
                <div class="col-lg  form-inline">
    ...
    methods: {
      ...
      saveTheCollectedData() {
        var _this = this;
      	...
      	for(var i=0;i<this.ourAccount.length;i++) {
          if(this.cashier.account===this.ourAccount[i]['short_name']) {
            this.cashier.id_account=this.ourAccount[i]['id'];
          }
      	}
...
console.log(response.data);
    ...
    beforeCreate:function() {
      ...
      this.$axios({
        ...
        _this.ourAccount=response.data;
      }).catch(function (error) {
        console.log(error);
        _this.$toast({
          text: '异步通信错误!'+error,
